...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    return _selectedStyle == MyselfCollectGoods ? 20 : _topicDatas.count;
}
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (_selectedStyle == MyselfCollectGoods) {
        ...
        topicCell.textLabel.text = _topicDatas[indexPath.row].title;
        [topicCell.imageView sd_setImageWithURL:[NSURL URLWithString:_topicDatas[indexPath.row].cover_image_url] placeholderImage:[UIImage imageNamed:@"zhanwei"]];
        return topicCell;
    }
}
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (offsetY < -headerViewHeight - 20 - 120) {
        self.contentOffset = CGPointMake(0, -headerViewHeight - 20 -120);
        return;
    }
